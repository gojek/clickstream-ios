...
class CourierEventBatchTests: XCTestCase {
    ...
    func testInitWithMultipleEvents() {
        let uuid = UUID().uuidString
        let event1 = CourierEvent(guid: "guid1", timestamp: Date(), type: "realtime", eventProtoData: Data(), expiryTime: Date())
        let event2 = CourierEvent(guid: "guid2", timestamp: Date(), type: "realtime", eventProtoData: Data(), expiryTime: Date())
    ...
    func testEventBatchCodable() throws {
        let uuid = UUID().uuidString
        let event = CourierEvent(guid: UUID().uuidString, timestamp: Date(), type: "realtime", eventProtoData: "test".data(using: .utf8)!, expiryTime: Date())
        let eventBatch = CourierEventBatch(uuid: uuid, events: [event])
